#!/usr/bin/env bash
# step20-patch.sh — scripts/spatialIntersect.js (spec §4.7 step ⑪), v2
# v2 fix: malformed-geometry PA/river features no longer crash the whole run — they're
# named, skipped, and excluded (their river_ids is left untouched, not wiped to []).
# Run from project root in Git Bash. Idempotent.
set -euo pipefail

mkdir -p scripts

cat > scripts/spatialIntersect.js << 'PATCH_EOF'
...
echo "Patch applied. Run: node scripts/spatialIntersect.js"
echo "Requires build/pa-merged.geojson (from mergeFeatures.js) and build/rivers-by-id/*.geojson"
echo "(from prepareRivers.js) to already exist on disk — both are gitignored build intermediates,"
echo "so this must run on your machine, not a fresh clone."
