#!/usr/bin/env bash
# Step 49 — font preload + preconnect on every page's <head> (spec §11.3)
# Requires Step 48 (public/_headers) already applied.
# Idempotent: scripts/addFontPreload.mjs marker-guards each file (skips if already patched).
set -euo pipefail
...
mkdir -p scripts
cat > scripts/addFontPreload.mjs << 'SCRIPTEOF'
...
node scripts/addFontPreload.mjs

pnpm install
pnpm build

echo "Step 49 complete: font preload + Cloudflare preconnect on all 11 page templates, pnpm build verified clean."
