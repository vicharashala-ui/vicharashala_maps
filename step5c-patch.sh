#!/usr/bin/env bash
# Step 5c: process the data.gov.in "Rivers" shapefile into matched geometry + partial metadata.
# Run from the project root.
#
# BEFORE running: copy your downloaded shapefile's 7 files (Rivers.shp/.dbf/.shx/.prj/.cpg/
# .sbn/.sbx) into build/raw/rivers-shp/ in this project.
set -euo pipefail
...
if [ ! -f build/raw/rivers-shp/Rivers.shp ]; then
  echo "Missing build/raw/rivers-shp/Rivers.shp" >&2
  echo "Copy your downloaded shapefile's files (Rivers.shp/.dbf/.shx/.prj/.cpg/.sbn/.sbx)" >&2
  echo "into build/raw/rivers-shp/ first, then re-run this script." >&2
  exit 1
fi
...
echo "==> Installing mapshaper (shapefile -> GeoJSON, reprojection)"
pnpm add -D mapshaper@0.7 || true
pnpm approve-builds better-sqlite3 msgpackr-extract || true

echo "==> Writing scripts/processRiversShapefile.js"
mkdir -p scripts
cat > scripts/processRiversShapefile.js << 'EOF'
...
echo "==> Running scripts/processRiversShapefile.js"
node scripts/processRiversShapefile.js

echo "==> Committing"
git add -A
git commit -q -m "Step 5c: government rivers shapefile processed — 61/105 rivers matched with basin/length metadata" \
  || echo "Commit skipped (configure 'git config user.name/user.email' then commit manually)."

echo ""
echo "==> Done. See build/rivers-govt-report.json for the full matched/unmatched/needs-verification breakdown."
