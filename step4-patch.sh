#!/usr/bin/env bash
# Step 4: State/UT boundaries — enrich with id/admin_type/capital (spec §4.7 step ⑨).
# Run from the project root. No compiled tools needed — safe on native Windows/Git Bash.
set -euo pipefail
...
echo "==> Writing scripts/enrichStates.js"
mkdir -p scripts
cat > scripts/enrichStates.js << 'EOF'
...
mkdir -p build/raw
cp /tmp/ecoguesser-src/scripts/source-data/india-states.geojson build/raw/india-states.raw.geojson
rm -rf /tmp/ecoguesser-src

echo "==> Running enrichStates.js"
node scripts/enrichStates.js

echo "==> Committing"
git add -A
git commit -q -m "Step 4: state/UT boundaries — enrichStates.js; 36 records (28 states + 8 UTs)" \
  || echo "Commit skipped (configure 'git config user.name/user.email' then commit manually)."

echo "==> Done. public/geojson/india-states.geojson and public/data/states.json are ready."
