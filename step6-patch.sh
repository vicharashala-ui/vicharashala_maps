#!/usr/bin/env bash
# Step 6 (v2 — fixed performance bug): join HydroRIVERS Strahler stream_order onto our
# named rivers. v1 hung indefinitely at real scale (500K+ reaches); this version uses a
# grid spatial index and was stress-tested at that exact scale before being handed over —
# see PROGRESS.md.
# Run from the project root.
#
# BEFORE running:
# 1. Download the "Asia" shapefile from https://www.hydrosheds.org/products/hydrorivers
# 2. Unzip it and place all its files into build/raw/hydrorivers/ in this project
set -euo pipefail
...
if [ ! -f build/raw/hydrorivers/HydroRIVERS_v10_as.shp ]; then
  echo "Missing build/raw/hydrorivers/HydroRIVERS_v10_as.shp" >&2
  echo "Download the Asia shapefile from hydrosheds.org/products/hydrorivers, unzip it," >&2
  echo "and place its files in build/raw/hydrorivers/ first." >&2
  exit 1
fi

if [ ! -f build/rivers-govt-matched.geojson ] || [ ! -f build/rivers-govt-metadata.json ]; then
  echo "Missing Step 5c output (build/rivers-govt-matched.geojson / rivers-govt-metadata.json)." >&2
  echo "Run step5c-patch.sh first." >&2
  exit 1
fi

# If a previous (v1, hung) run left a partial clip file, it's still valid to reuse — the clip
# step itself wasn't the slow part. Only delete it if you want to force a re-clip.

echo "==> Installing dependencies (safe if already installed)"
pnpm install

echo "==> Writing scripts/joinHydroRivers.js"
mkdir -p scripts
cat > scripts/joinHydroRivers.js << 'EOF'
...
echo "==> Running scripts/joinHydroRivers.js"
echo "    (stress-tested at 500K-reach scale beforehand: ~28s total. If this run takes"
echo "    dramatically longer, something's off — let me know rather than waiting it out.)"
node scripts/joinHydroRivers.js

echo "==> Committing"
git add -A
git commit -q -m "Step 6: HydroRIVERS Strahler stream_order joined onto named rivers (grid-indexed)" \
  || echo "Commit skipped (configure 'git config user.name/user.email' then commit manually)."

echo ""
echo "==> Done. See build/hydrorivers-join-report.json for per-river match details."
